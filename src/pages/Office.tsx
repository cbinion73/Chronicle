...
function WelcomeBackCard({ prayerItems, lastEntry, card }: {
    ...
    <section style={{ ...card, borderColor: 'var(--accent-primary)', background: 'var(--accent-primary-light)' }}>
      ...
      {carried.length > 0 ? (
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 5 }}>
            {carried.map((item) => (
              <div key={item.id} style={{ fontSize: 13, color: 'var(--text)', fontFamily: 'var(--font-serif)' }}>
                · {item.text}
              </div>
            ))}
          </div>
      ...
      {lastEntry ? (
        <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: '12px 0 0' }}>
          The last thing you wrote: <em>"{lastEntry.title}"</em>
        </p>
      ) : null}
    </section>
  );
    ...
    return (
      <div className={chapelStyles.chapelRegister} style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        <div style={{ maxWidth: 640, margin: '0 auto', padding: isPhone ? '22px 16px 48px' : '36px 24px 64px', display: 'grid', gap: 16 }}>
          ...
          {/* 1 · Review */}
          <section style={card}>
            <StationLabel n={1}>The Day's Thread</StationLabel>
          ...
          {/* 2 · Silence */}
          <section style={card}>
            <StationLabel n={2}>Silence</StationLabel>
            {silenceDone ? (
          ...
          {/* 3 · Response */}
          <section style={card}>
            <StationLabel n={3}>Give the Day Back</StationLabel>
            <textarea
              value={response}
